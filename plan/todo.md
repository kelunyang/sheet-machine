...
### 32. 輸出 PDF：問卷列表 P 欄範本＋送出時產生＋登入頁「取得我的 PDF」（2026-09-16 設計定案，規格見 plan.md Phase 31）✅

**2026-09-16 實作完成（未部署）**：問卷列表新增 P 欄「輸出PDF」（Google 文件範本 ID）。送出成功後
複製範本、兩輪替換 `{{欄位ID}}`、轉 PDF 存進 `pdfFolderID` 資料夾（每人一檔、檔名 HMAC、重新送出用
進階 Drive 服務覆蓋同一檔），結束頁給連結；登入頁「查詢是否填寫」換成「取得我的 PDF」（與查詢共用
抽出來的 `readonlyAuthGate_` 認證骨架，最新版直接回連結、否則重產）。`tools/export.js` 讀寫 16 欄、
設定對話框加欄位、檢查器檢查範本佔位符。管理者工具 `listRecordPdfs()`。lint／test（539 綠，新增 45 例）／
build（219.00 KB）全過。**同日 code review 修正**（詳 plan.md Phase 31「Code review 修正」）：共用電腦殘留連結、
P 欄備註誤開功能、去重丟掉已發連結、共用雲端硬碟 404、舊版蓋新版、鎖範圍過大、產生次數無上限（新增
`pdfGenMax`／`pdfGenWindowMinutes`）、佔位符被切開默默失敗，以及四項效能與改用 `getUrl()`；
lint／test（550 綠）／build（219.61 KB）全過，主要修正皆以故意改壞驗證測試抓得到。

- **起因**：填完要印出來核章的問卷，原本要管理者自己套版。「核章後上傳」拆成另一份問卷（現有 F-F 欄），
  PDF 一定用已送出的紀錄產生，紙本與紀錄天生一致。
- **決策過程**（與維護者逐條討論）：連結分享（非系統下載，取捨見 issue.md）、檔名 HMAC＋獨立不可輪替的
  `pdfNameSecret`、版本覆蓋、對照不落試算表、簽名圖支援但不強制、有 PDF 就不需要「查詢是否填寫」。
- **待人工驗收（無法單元測試的部分）**：
  1. 啟用進階 Drive 服務後重新授權；設 `pdfFolderID` 並開連結分享。
  2. 實機範本：一般欄位／名冊欄／`{{送出時間}}`／`{{簽名:…}}`（段首與段中）／多行文字／含 `$` 的值／表格內佔位符／頁首。
  3. 重新送出 → **連結不變**、「管理版本」有舊版；`makeCopy` 是否計入「建立文件」每日額度。
  4. 改範本後「取得我的 PDF」→ 重產新版面；暫存文件確實進垃圾桶、不在分享資料夾。
  5. 舊 15 欄問卷列表不補 P 欄 → 全站照常。
  6. `tools/export.js` 手動貼進 container-bound 專案，測「修改問卷設定」的輸出PDF 欄與「檢查問卷格式」（首次會要求 DocumentApp 授權）。
  7. 選單「檢查PDF附掛文件」（2026-09-17 補）：實機驗證被超連結／智慧型方塊切開的佔位符真的被 `findText` 判成切開
     （單元測試只能用假文件模擬，`findText` 與 `replaceText` 逐段比對行為一致是依文件推論）。
     同日修正舊檢查的誤判：`{{簽名: 家長}}`（冒號後有空白）舊檢查說沒問題、後端其實換不到；對照表單 A1 空白時後端整張沒欄位。

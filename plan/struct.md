...
| `inviteeLogin(token, otp)` | 邀請碼 + OTP | ScriptLock 內比對 hash（計次/作廢），通過才簽發受邀者 session JWT（exp = min(1hr, 邀請到期, dueDate)）+ read-only headers（已送出紀錄疊線上暫存草稿）；OTP 錯誤回 `{otpFailed}` |
...
│   │   ├── SheetCard.vue          # 問卷列表卡片（右上角狀態＋靜態流程看板串）
...
│   │   ├── sheetFlow.js           # 問卷卡片純函數（看板串組成/右上角狀態）
...
- **TempTransferDrawers**：匯出/匯入暫存檔的兩個 drawer。
...
- **SignatureToolbar**：簽名確認 drawer 的 sticky 控制列（比照 FormToolbar）：JWT 條＋
  ...
  只剩 JWT 條。主行動「送出簽名」與「回去修改」同樣收斂到 drawer footer（.formFooter），
...
- **InviteeSignDialog**：受邀簽名者的簽名畫面：邀請碼＋OTP 二段驗證在 App.vue 的
  ...
  read-only 問卷 + 單格簽名，自建 useSignatures 實例。
...
- **SheetCard**：問卷列表卡片，取代舊 el-table（Phase 14）：版面骨架挪用
  scoringSystem-cf ProjectCard——標題＋右上角兩行色點期限（「可填寫至」帶緊迫度
  success/warning/danger 色、「可檢視至」固定 info 灰；已截止/暫時關閉/不開放時
  第一行換狀態文字）＋靜態流程看板串「開始(建立日期)→填寫→簽名×n(有簽名格才出現)
  →結束(dueDate)」，起訖日期掛在格子外下方。問卷沒有「當前階段」所以不搬
  LED/matter.js/小人，期限開卡算一次不自跑 tick；組成純函數在 utils/sheetFlow.js。
...
- **useDraft**：線上暫存 save/load/delete + saveDraftForInvite。

...
```javascript
{
  ...
  value: "當前值",        // 使用者輸入的值
  savedContent: "原始值", // 名冊母表（referSSID）該人該欄的值＝「系統儲存的答案」
  lastInput: "上次輸入",  // 紀錄表（recordSSID）該主鍵最後一列的值＝「上次送出的答案」
  status: "",             // 驗證錯誤訊息
  tid: "uuid",            // 前端用的唯一識別碼
  uploadUrl: "https://…", // 檔案欄：**這次** saveFile 上傳的檔案連結（lastInput 是上次送出的，
                          // 前端永不覆寫它——覆寫過去就是 Phase 23 實機回報的 diff 前後同檔 bug）
  // ↓ Phase 23：答案來源切換器（FieldValueSwitch 的 el-segmented）的狀態
  draftOrigin: { val: "疊回時的值", source: "local|online|import" }, // 「暫存」這個來源的內容
  userInput: "使用者自己填的值",  // 「你現在填的」——markUserInput() 在使用者動手時寫入
  source: "saved|last|draft|user" // 目前生效的是哪個來源（切換器與 markUserInput 都會寫它）
}
```

**每題的四個答案來源**（Phase 23 起在 UI 上是一條 el-segmented，切了就自動帶入）：

| 選項 | 來源欄位 | 從哪來 |
|------|----------|--------|
| 預設值 | `savedContent` | 名冊母表該人該欄（readRecord 給） |
| 你上次的 | `lastInput` | 紀錄表該主鍵最後一列＝上次送出（readRecord 給） |
| 暫存 | `draftOrigin.val` | localStorage／線上 `_draft`／匯入檔疊回（columnPrep／tempQueue 標） |
| 你現在填的 | `userInput` | 使用者自己動手填的（markUserInput 寫入；沒填過這個選項是 disabled） |

沒有值的來源不長出選項；`value` 永遠是「當前生效的那個來源的值」。切去看別的來源再切回
「你現在填的」不會弄丟使用者填的東西（`userInput` 獨立保存）。
...
## 檔案欄位的 fileID 歸屬驗證（2026-07 Phase 23）

**補掉的缺口**：舊版 writeRecord 對檔案欄「原樣採用前端傳來的 fileID」且不驗歸屬——任何人都能把
不屬於自己的 fileID 掛進自己的紀錄，回條信的 `DriveApp.getFileById` 還會幫忙轉成可開的 URL
（等於探測＋取得他人檔案的通道）。根因是上傳的 fileID 從來沒地方登記、後端無從驗證。

### `_file` 分頁（存於 draftSheetID 試算表）

純 append 上傳登記，6 欄：A `timestamp(ms)`、B `referSSID`、C `uploader`（id 假名，
`deriveDraftKey_` purpose='id'，與 `_draft` A 欄同源）、D `columnID`（**欄位 ID，不是位置**——
pos 會隨管理者在對照表單插欄/搬欄而位移，舊登記列就會對到別的欄位）、E `fileID`、F `mimeType`。
第 1 列表頭 `FILE_HEADER`（凍結、對 reader 惰性）；建分頁權只在寫入路徑（`appendFileLog_`←saveFile，
draftSheetID 未設則靜默不記）。只有 metadata，無檔案內容、無明文個資。鐵律照舊：純 append、
禁 deleteRow、ms timestamp；離線重建對認不得的分頁原樣照抄，`_file` 走此 fail-safe。

### 兩條寫入路徑

1. **沿用上次上傳的檔案**：前端只送哨兵 `__SM_REUSE_LAST_FILE__`（`utils/sentinels.js`／Code.js
   同字面常數，測試鎖），**前端沒有傳舊 fileID 的通道**；writeRecord 遇哨兵才用
   `latestRecordRowFor_`＋`resolveReuseFileId_` 從**本人**（claims.pkey，token 裁決）紀錄表最後一列
   查出真 fileID 填入，查無整筆擋下。哨兵絕不落地（比照簽名的 `resolveSignatureSources_` 模型）。
   **不 fallback 名冊**——名冊母表存的是檔名片段（readRecord 用 `title contains` 模糊搜），
   模糊搜的結果歸屬無法保證，不可當寫入來源。
2. **新上傳**：`fileLogHasUpload_` 查 `_file` 有無同 (referSSID, 上傳者假名, **columnID**, fileID)
   的登記列；查無時 fallback 比對本人紀錄表最後一列同欄 fileID（自家落過地的值，涵蓋 `_file`
   上線前的舊暫存）；兩者皆無 → 整筆擋下「檔案來源無法確認」。

**誠實邊界**：draftSheetID 未設時整段驗證跳過（無登記表可查，維持舊行為——缺口仍開，比照
...
│   ├── components/
│   │   ├── FormField.vue          # 單一問卷欄位（左側狀態邊界條 + 答案來源切換器 + 各 format 輸入元件）
│   │   ├── FieldValueSwitch.vue   # 每題的答案來源 el-segmented（預設值/你上次的/暫存/你現在填的，切了自動帶入）（Phase 23）
│   │   ├── SubmitDiffDrawer.vue   # 送出前差異對照 drawer（簽名之前，零差異自動跳過）（Phase 23）
│   │   ├── DiffText.vue           # 兩段純文字逐行 diff（diff + diff2html，手機切 line-by-line）（Phase 23）
...
- **FormField**：單一問卷欄位，各 format 的輸入元件 + 驗證顯示（就地改寫 column.status/value）。
  **左右兩欄（Phase 23）**：左欄＝狀態邊界條——一條狀態色的細線（撐滿題高＝題與題的視覺界線，
  長問卷裡原本黏成一片）＋**頂端一組 FA 標記圖示**：填答狀態（success `fa-circle-check`／danger
  `fa-circle-exclamation`／warning `fa-circle-half-stroke`／info `fa-circle-dot`，來自
  `statusDetector`），其下接「與上次送出不同」（`fa-pen`，`differsFromLast` 且可編輯時才出現，
  固定 warning 色不跟狀態色走——它講的是「這次改了什麼」，與「填得對不對」是兩回事）。
  兩個圖示的說明都走 el-tooltip＋`aria-label`，不佔版面（取代原本每題頂部的 el-tag 與
  segmented 旁的差異 tag）；
...
- **FieldValueSwitch**（Phase 23）：每題的**答案來源切換器**（`el-segmented`）——
  預設值／你上次的／暫存／你現在填的，**切換就自動把該來源的值帶進輸入框**（不必再按帶入鈕；
  per-format 轉換走 `utils/fieldSources.js` 的 `coerceSourceValue`）。沒有值的來源不長出選項；
  「你現在填的」永遠列出但**使用者真的動手前是 disabled**（`hasUserInput`）。只有一個選項時
  整條不顯示。使用者一動手（打字/選單/滑桿/上傳/多選/郵遞區號/「無資料」鈕）就 `markUserInput()`：
  把當下的值存進 `column.userInput`、來源切回 user——**切去看預設值再切回來不會弄丟自己填的東西**。
  檔案欄特化：「你上次的」帶入的是哨兵 `__SM_REUSE_LAST_FILE__` 而非 fileID（伺服器端查）；
  「預設值」不長出選項（名冊存的是檔名片段，後端無從安全解析）；值進不了輸入框，改在切換器
  下方直接顯示該來源的檔案連結。登入時若有暫存疊回，`column.source` 就是 'draft'（一進場停在「暫存」）。
  **「與上次送出不同」的標記不在這裡**——它與填答狀態圖示同一組，掛在 FormField 的左側狀態邊界條上。
- **SubmitDiffDrawer**（Phase 23）：送出前的差異對照。authMod 驗證通過後、簽名／確認 drawer 之前
  的**必經**一步，但 `hasAnyDiff` 為 false 時 App.vue 直接跳過不開（零差異不打擾）。
  文字題走 DiffText 逐行 diff、檔案欄走前後連結對照（比不了內容）；比對基準用 **el-segmented**
  （與題目上的來源切換器同一種控制項）在「你上次的」（預設；沒送出過自動退回預設值）與
  「預設值」之間切。footer 兩顆按鈕各半寬（比照填問卷 drawer 的 `.formFooter__buttons`：
  次要動作「回去修改」靠前、主要動作「繼續下一步」靠後）。
- **DiffText**（Phase 23）：兩段純文字的逐行 diff（`diff` 的 createTwoFilesPatch → `diff2html`
  的 html，兩者走 CDN import map；CSS 走 index.html 的 `<link>`，**JS 與 CSS 版號兩處必須同步**）。
  搬自 scoringSystem-cf 的 RankingComparison.vue 並改成吃兩段字串。桌機 side-by-side、
  **手機（≤768px，matchMedia）自動切 line-by-line**；輸出過 DOMPurify 才 v-html（全站紅線）。

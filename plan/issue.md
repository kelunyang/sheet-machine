...
## 主鍵寫進試算表一律帶 📝，讀回一律先剝（2026-09-17）

**刻意設計，要推翻先討論。**

- **事故**：116 部署的「基本資料調查表」主鍵是學校代碼（`011310`）。送出後紀錄表 C 欄變成 `11310`——
  `appendRow` 寫進去的純數字字串會被試算表自動轉成數字，開頭的 0 就沒了。名冊是 `011310`，所以
  填答率永遠 0，而且同一個原因也讓「你上次的答案」「查詢我填答了沒」「沿用上次的檔案」「取得我的 PDF」
  全都找不到這個人。**把紀錄表 C 欄格式設成純文字擋不住**（維護者實測，新列照樣被吃 0）。
- **做法**：比照答案欄既有的 📝，主鍵寫進試算表時加前綴 `📝`，讀回比對前剝掉。
  單一入口：`Code.js` 的 `pkeyCell_`（寫）／`pkeyFromCell_`（讀）；`tools/export.js` 是另一個 GAS 專案、
  叫不到 Code.js，另放一份 `pkeyFromCell_`（字面值有測試鎖，`tests/pkeyMarker.test.js`）。
- **涵蓋的欄**：紀錄表 C、`_invites` D、`_logins` C、`_email` F。**新增任何會把主鍵寫進試算表、
  或從試算表讀主鍵回來比對的程式，都要走這兩支**——主鍵到處都是，漏一處就是「那個功能對 0 開頭的人
  靜默失效」，不會報錯。
- **📝 只活在儲存格裡**：剝完的原值才能拿去比對、算 HMAC（暫存假名、PDF 檔名）、簽 JWT、組檔名、寄信。
  名冊不加（名冊是管理者手打的，而且登入 `getUserRow` 拿名冊原值比）。
- **相容**：`pkeyFromCell_` 只剝開頭一個 📝，沒有前綴的舊列原樣——上線前送出的紀錄照樣認得，同一人
  新舊列算同一個主鍵。**已經被吃掉 0 的舊列救不回來**（不知道原本有幾個 0），要人工改成 `📝011310`。
- **匯出**：`tools/export.js` 分組前先剝 📝（同一人新舊列才歸同一組、名冊回填才查得到），輸出時主鍵
  一律帶 📝——C 欄主鍵，**以及從名冊回填進答案區的主鍵題（type P，不論 format）**。輸出也是 `setValues`，
  不帶會再被吃一次；原本名冊回填只有 L|N|M|P|G 格式才帶 📝，P-T 的學校代碼就漏了。匯出完成的提醒本來
  就要管理者自己去掉數字前的 📝。
- **部署順序：`tools/export.js` 要比 web app 早換（或同時）**。export.js 是手動貼進問卷列表母表的另一個
  專案，新版對沒標記的舊列完全相容；反過來 web app 先上、匯出工具還是舊版，同一人會被匯成
  「S001」「📝S001」兩列，而且新列查不到名冊、回填欄位空白。
- **倒回舊版 web app 不乾淨**：新版寫下的 📝 列，舊程式認不得（填答率、上次答案、邀請全部對不上）。
  真要倒回，得連同把這段期間的 📝 人工去掉。
- **repo 以外**：管理者自己在試算表寫的公式（`COUNTIF`／`VLOOKUP` 紀錄表 C 欄、`_logins` C 欄）
  對新列會失效，要先 `SUBSTITUTE(…,"📝","")`。
- **沒動的**：紀錄表 E 欄（分組值）同樣會被吃 0（如班級 `01`），`compareSheets` 分組模式目前還拿它配對；
  Phase 29 規格本來就要改成只用主鍵配對、不讀 E 欄，那件事另外處理。

...
### 卡片版面（骨架照抄 ProjectCard，2026-07-10 補充定案；右上角 2026-07-11 改兩行）

```
問卷名稱（過長 ellipsis）    ● 可填寫至 07-15 17:00   ← 右上角兩行：色點＋期限
[tags]                      ● 可檢視至 08-01 00:00
[開始]──→[填寫]──→[簽名 ×2]──→[結束]
07/01                            07/15 17:00
                                     [填寫&檢視表單]
```

**右上角兩行期限（挪用 ProjectCard 的 status-dot 機制）**：
第一行「可填寫至」帶緊迫度顏色，第二行「可檢視至」固定 info 灰——
檢視期上移到這裡後，原本卡底的「截止後仍可檢視至」細字退役，UI 一致
（2026-07-11 調整定案，取代原「剩餘倒數」單行狀態）：

| 情境 | 第一行文字 | 色 |
|------|------|----|
| 開放填寫中（active） | 可填寫至 YYYY-MM-DD HH:mm | success 綠 |
| 剩 <10 分鐘（warning） | 可填寫至 YYYY-MM-DD HH:mm | warning |
| 已截止仍可檢視（ended） | 已截止填寫 | danger 紅 |
| `writeAllowed=false` | 暫時關閉 | info 灰 |
| `dueDate=0` | 不開放填寫 | info 灰 |

第二行「可檢視至 YYYY-MM-DD HH:mm」（getQList 只回傳 viewDate 未過的問卷；
`viewDate<=0` 資料異常、或 `viewDate===dueDate`（截止即不可檢視、第二行冗餘）時整行不顯示）。
...
- **結束標記**：「結束」＋`dueDate` 日期；`dueDate=0` 顯示「不開放」
- 箭頭用 CSS/unicode（專案沒有 FontAwesome），配色吃主題變數；
  已截止整條轉灰＋結束標記 danger 紅字；`writeAllowed=false` 整條灰＋維持「本問卷暫時關閉」
- 無任何動畫

### 前端

- [ ] 純函數 `src/utils/sheetFlow.js`：`buildFlowChips(sheet, now)` 回
      `[{ type: 'start'|'chip'|'end', label, sub?, state }]`、
      `sheetStatus(sheet, now)` 回右上角兩行
      `{ write: { text, tone }, view: { text, tone }|null }`
      ——組成/文案/狀態全在純函數，供單元測試
- [ ] 新元件 `src/components/SheetCard.vue`：props `sheet`，emit `open`；
      頂列名稱（ellipsis）＋右上角兩行期限（見上表）、tags（沿用 tagPalette）、
      中段看板串、底列進入按鈕

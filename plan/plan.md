...
## Phase 31：輸出 PDF——問卷列表 P 欄「輸出PDF」＋送出時產生＋登入頁取得（2026-09-16 設計定案）

### 動機

有些問卷填完要**印出來拿去核章**（家長、導師、行政單位蓋章），再到另一份問卷上傳核章後的掃描檔。
過去要管理者自己拿紀錄表套版、一份一份產生。本 Phase 讓問卷在送出時自動用 Google 文件範本
產生 PDF，填寫者當場拿連結，事後也能在登入頁重新取得。

「核章後上傳」**拆成另一份問卷**（用現有的 F-F 上傳欄），不在本 Phase 範圍——這樣 PDF 一定是
用「已送出、已驗證」的紀錄產生，紙本內容與紀錄天生一致，不必處理「產生後又改答案」。

### 定案表（2026-09-16 與維護者逐條討論）

| 決策點 | 選擇 |
|---|---|
| 設定位置 | **問卷列表 P 欄「輸出PDF」**＝Google 文件範本 ID，留空＝不輸出。加在尾端，既有欄位不位移（`plan/2026-summer.md` 允許的擴充方式） |
| 產生時機 | **送出成功後**由後端產生（`writeRecord_` appendRow 之後）；**產生失敗不擋送出**（紀錄已寫入），前端提示稍後到登入頁補拿 |
| 事後取得 | 開啟 PDF 的問卷，登入頁藍色「查詢是否填寫」**換成**「取得我的 PDF」（維護者判斷：有 PDF 就不需要查詢功能）；沒有送出紀錄＝警告「需完成填寫並送出後，才可產生 PDF」 |
| 交付方式 | **資料夾開「知道連結可檢視」，回傳檔案連結**，與 F-F／C-F 欄位同模型（維護者決定，見下方誠實邊界） |
| 存放 | ScriptProperties **`pdfFolderID`**；每人每份問卷**只有一個 PDF 檔** |
| 檔名 | `base64url(HMAC-SHA256(pdfNameSecret, JSON.stringify(['pdf', refer, record, 主鍵])))` + `.pdf`：好搜、檔名不露主鍵、不能從問卷 ID 窮舉反推主鍵 |
| 檔名密鑰 | 新增 **`pdfNameSecret`**（首次自動生成，比照 `jwtSecret`），**不可輪替**；不共用 `jwtSecret`（要能隨時換）或 `draftEncSecret`（暫存出事時要換） |
| 重新送出 | **Drive 版本覆蓋同一檔**（進階 Drive 服務 `Drive.Files.update`）：檔案 ID 與連結不變，舊版留在「管理版本」 |
| 簽名圖 | **支援但不強制**：範本放 `{{簽名:格名}}` 才插圖，沒放不影響 |
| 對照記錄 | **不另開試算表分頁**：靠檔名（HMAC）找檔，版本新鮮度記在檔案 description——不依賴 `draftSheetID`、不怕暫存表重建或 `draftEncSecret` 輪替 |

### 實作規格

#### 範本語法（Google 文件，內文／頁首／頁尾都認）

| 佔位符 | 印出 |
|---|---|
| `{{欄位ID}}`（type F） | 紀錄列該欄；`📝` 剝除；多選 `a;b` → `a、b`；檔案欄 fileID → Drive 連結（多檔空白分隔）；「不提供資料」「無資料」照原樣印 |
| `{{欄位ID}}`（type P） | 伺服器判定的主鍵值 |
| `{{欄位ID}}`（type A／O／G、C-T） | 產生當下名冊列該欄 |
| `{{送出時間}}` | 紀錄列 A 欄，`yyyy/MM/dd HH:mm:ss`（腳本時區） |
| `{{問卷名稱}}` | 問卷列表 A 欄 |
| `{{簽名:格名}}` | 紀錄列 D 欄依問卷列表 G 欄格名順序取 fileID，插成寬度上限 180px 的圖（全形冒號亦可） |

C-M／C-S／C-F **沒有可印的值**（C-S 計算結果不落地，見 issue.md），這類佔位符與打錯的 ID 一樣原樣留在
PDF 上、`console.warn` 記一筆；「檢查問卷格式」會事先警告。欄位 ID 與系統鍵撞名時欄位優先。

#### 後端（`src/Code.js`）

1. **`listValues_(listSheet)`**：`getMaxColumns() >= 16` 讀 `A:P`，否則讀 `A:O`。**舊表只有 15 欄時
   直接 `getRange("A:P")` 會丟例外讓整站掛掉**，所以不能無條件改讀範圍。`getQList_`、`writeRecord_`、
   認證骨架改用它；其他只用到 A~O 的讀取點不動。
2. **`readonlyAuthGate_`**：把 `mySubmitStatus_` 的認證骨架（冷卻 → `authRecord` → `_logins` → N 欄
   「開放進入」→ 伺服器判定主鍵 → 名冊有這個人）抽成共用函數，`mySubmitStatus_` 與新的
   `myRecordPdf_` 都走它——新入口字面上共用同一段程式，不可能變成防枚舉旁路。名冊列一律用
   伺服器判定的主鍵查（`authArrayFromClaims_`），不用前端傳來的值。
3. **純函數**（可 vitest）：`pdfPlaceholders_`（抽出 `{{…}}`）、`pdfValueMap_`（組值）、
   `pdfMissingKeys_`、`pdfDescription_`（新鮮度字串）、`pdfTemplateOf_`（P 欄，舊表 undefined → 空字串）。
4. **填值 `fillPdfDocument_`**：兩輪。第一輪把範本佔位符換成**這次產生專用、帶亂數 nonce 的記號**；
   第二輪才逐一把記號換成值（先插值再刪記號，避免整段文字刪光後元素被移除）。填寫者在文字欄打的
   `{{P01}}` 是第二輪才進文件，不會被當成佔位符再換一次。簽名圖插在記號所在文字段**之前**
   （記號在段首）或**之後**（其他位置，不拆字串）——範本建議把簽名佔位符單獨放一行或一格。
5. **產生 `generateRecordPdf_`**：範本 `makeCopy` 到部署帳號**根目錄**（不進分享資料夾）→
   `DocumentApp` 填值 → `saveAndClose()`（少這行 PDF 是替換前的內容）→ `getAs('application/pdf')`
   → `finally` 暫存文件丟垃圾桶。存放在 `ScriptLock` 內：資料夾找同名檔（多個取最新、其餘丟垃圾桶），
   有就 `Drive.Files.update` 覆蓋並更新 description，沒有就 `createFile`。未啟用進階 Drive 服務直接丟錯。
6. **新鮮度**：description＝`sheet-machine PDF tick=<送出ms> template=<範本ID>@<範本最後修改ms>`。
   登入頁取得時字串完全相同才直接回連結；**送出時間或範本有變就重新產生**（範本改版後舊 PDF 會在
   本人下次取得時自動換新版面）。
7. **`writeRecord_`**：P 欄有值時產生，回傳多 `pdf: { url } | null`、`pdfError: boolean`；回條信多一行 PDF 連結。
8. **新 RPC `myRecordPdf(referSSID, recordSSID, auth)`**：認證不過回 `false`／冷卻形狀；P 欄空
   `{pdfDisabled}`；無紀錄 `{noRecord}`；產生失敗 `{failed}`；成功 `{url, lastTick, generated}`。
   也涵蓋「功能中途才開啟、之前送出的人還沒有 PDF」與「送出時產生失敗」的補拿。
9. **管理者工具 `listRecordPdfs()`**（編輯器手動執行）：檔名是 HMAC、從資料夾認不出誰是誰，這支把
   每份有開 PDF 的問卷、每人最新送出對應的連結印到執行紀錄。它是 public 函數（編輯器只能跑沒底線的），
   google.script.run 也叫得到，所以開頭擋「執行者必須是部署帳號本人」、不回傳任何東西。
10. **`appscript/appsscript.json`** 加 `enabledAdvancedServices` Drive v3。

#### 前端

- `src/utils/recordPdf.js`：`pdfResultKind(res)` 分類回應＋`PDF_MESSAGES` 文案（純函數，有測試）。
- `App.vue`：`currentSheet.pdfEnabled` 時第二顆按鈕換成「取得我的 PDF」（掛 loading 小遊戲、
  冷卻共用登入倒數），成功顯示「開啟 PDF」連結與版本時間；結束頁顯示「開啟 PDF」或產生失敗提示。
  換問卷時清空兩者。

#### `tools/export.js`

- 列表讀寫改 16 欄（舊表存檔前補到 16 欄，只在尾端加欄）；「新增問卷」P 欄留空；
  「修改問卷設定」對話框加「輸出PDF」欄位。
- 檢查器：P 欄 ID 格式；範本打不開＝錯誤；佔位符對不到欄位 ID／系統鍵／簽名格名＝警告；
  引用 C-M／C-S／C-F＝警告。

### Code review 修正（2026-09-16，同日）

實作完後跑 code review，維護者要求只修「程式會做錯事」的與效能類，以下都已補測試（每條都用故意改壞的方式確認測試抓得到）：

| 問題 | 修法 |
|---|---|
| 登入頁拿到的 PDF 連結換人後還留在畫面上（共用電腦看得到上一個人的 PDF） | `myPdf` 在認證欄位任何變動（換人輸入、登入、送出、結束時清空）時收掉；`loginView`、`endView` 也明確清 |
| P 欄只要非空就當範本 ID，寫個備註就打開 PDF 功能、每次失敗 | `pdfTemplateOf_` 只認 `^[-\w]{25,}$`（與檢查器同規則），其他一律當沒設 |
| 同名檔去重留最新的，已經發出去的連結被丟垃圾桶 | `existingPdfFile_` 改留**最早建立**的（連結最早發出去的那個） |
| 資料夾在共用雲端硬碟時覆蓋一律 404 | `Drive.Files.update` 帶 `{ supportsAllDrives: true }` |
| 比較慢的舊版產生蓋掉剛存好的新版 | 覆蓋前 `pdfTickOf_` 解析現有 description 的送出時間，現有的比較新就不寫 |
| 鎖包住整段 Drive 讀寫，尖峰拖垮線上暫存等只等 10 秒的功能 | 先不上鎖找檔，有就直接覆蓋；只有要新建時才拿鎖、鎖內再找一次 |
| 送出沒有次數限制，拿有效 token 反覆送出可燒光全站建立文件額度 | `claimPdfGeneration_`：每人每份問卷在窗口內最多產生 `pdfGenMax` 次（預設 10 次／`pdfGenWindowMinutes` 360 分，CacheService 上限 6 小時）；超過時送出照常、回 `pdfThrottled`，登入頁回 `{pdfThrottled, retryMinutes}`；「已是最新版直接回連結」不算次數 |
| 佔位符被超連結等格式切開時替換默默失敗 | 第一輪後檢查記號有沒有真的出現，沒有就列入回傳與 `console.warn` |
| （效能）沒開進階 Drive 服務時先整套產生才失敗 | 檢查移到 `pdfTarget_` 最前面 |
| （效能）每個佔位符對內文、頁首、頁尾都跑一次替換 | 只對文字裡真的有這個佔位符的區段呼叫 |
| （效能）一次查詢讀名冊 9～10 遍 | 骨架重用 `draftKey_` 結果、名冊只讀一次並回傳 `headers` 給呼叫端（剩 5 次，其中 authRecord 的 3 次與 readRecord_ 共用未動） |
| （效能）`listRecordPdfs` 每人讀一次密鑰 | `pdfFileName_` 可傳入先讀好的密鑰 |
| 連結自己拼網址 | 一律用 Drive 回的 `getUrl()`（需要時自帶 resourcekey）；範本裡的檔案欄也是，檔案不見印「（找不到檔案）」不讓整份失敗 |

維護者判斷**不修**：文字欄內容被試算表自動轉型（文字欄不會填數字）、「開放進入＝否會擋掉 PDF 補拿」的操作說明（管理者就是維護者本人）。
**改不了**：送出後改了簽名格順序，重產的簽名圖會對錯格（紀錄表沒存送出當時的格名）。

### 誠實邊界

- **連結分享的代價（維護者已知並接受）**：拿到**資料夾**網址的人能看到所有人的 PDF；檔案連結永久有效、
  可匿名開啟、系統查不到是誰看的。檔名 HMAC 只藏主鍵，不藏內容。
- **`pdfNameSecret` 換掉或遺失**＝每個人下次產生都找不到舊檔、另建新檔，**舊連結停在舊版**。
- **版本保留**：依 Google Drive 說明，一般檔案舊版本 30 天後或超過 100 個版本會被清，除非標「永久保留」。
- **額度**：每次產生（送出、或範本改版後的取得）用掉一份「建立文件」額度，官方配額表個人帳號
  250 份／天、Workspace 1,500 份／天，全系統共用；`makeCopy` 是否計入要實測。
- **名冊值取產生當下**：名冊改了姓名，要等下次產生才會反映。
- **計算欄印不出來**：要印金額得先把運算式搬到後端重算，另開 Phase。
- **無法單元測試的部分**：`DocumentApp` 填值、`makeCopy`／`getAs`／`Drive.Files.update` 只能實機驗證。

### 驗收

- `npm run lint`、`npm test`、`npm run build` 全過。
- 實機：
  1. 範本放一般欄位、名冊欄、`{{送出時間}}`、`{{簽名:…}}`、多行文字、含 `$` 的值 → 送出 → 結束頁連結打得開、內容正確、暫存文件已進垃圾桶。
  2. 同一人修改後重送 → **連結不變**、內容更新、「管理版本」有舊版。
  3. 登入頁「取得我的 PDF」：已送出 → 直接回連結（不重產）；改範本後再取得 → 重產新版面；沒送出 → 警告；認證錯 5 次 → 冷卻倒數。
  4. 文字欄輸入 `{{P01}}` → PDF 上照原樣印出、不被換成主鍵。
  5. 舊 15 欄問卷列表不加 P 欄 → 全站照常。
  6. `listRecordPdfs()` 在編輯器執行有輸出；從前端 console 以 `google.script.run.listRecordPdfs()` 呼叫無效果。

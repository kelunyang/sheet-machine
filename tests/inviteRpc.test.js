...
// 完整 GAS stub 環境；inviteRows / draftRows 是活的（RPC 寫入會改動它，測試可直接斷言）。
// draftRows = null（預設）＝ _draft 分頁不存在（getSheetByName 回 null，寫入路徑才建）
function loadGasRpc({
  ...
  // _draft 分頁 stub（Phase 20 假名化測試用）：draftStore 是活的、變長列讀取時補 '' 對齊。
  // draftRows = null 時 getSheetByName 回 null（分頁不存在），saveDraft 的寫入路徑 insertSheet
  // 才把它建出來（比照真機 draftSheet_ 行為，含 DRAFT_HEADER＋凍結首列）
  const draftStore = draftRows === null ? [] : draftRows;
  const makeDraftSheet = (rows) => ({
          ...
          getSheets: () => [
            {
              ...
              getDataRange: () => ({ getValues: () => recordRows.map((row) => [...row]) }),
            },
          ],
        };
      }
  ...
  const factory = new Function(
    ...
    `${source}\n;return {
      ...
      getDraftEncSecret_, deriveDraftKey_, saveDraft, loadDraft, readRecord_, DRAFT_HEADER
    };`
  );
  const gas = factory(
    { load: () => _ },
    ...
    // Phase 21：readRecord_ 走 checkLoginThrottle_/recordLoginAttempt_，需 CacheService（無 TTL 的簡易 Map 足夠）
    (() => {
      const map = new Map();
      const cache = {
...
// ===== Phase 20：暫存內容端到端加密（HMAC 派生金鑰＋_draft 假名化＋smd1 直通） =====

describe('deriveDraftKey_ / getDraftEncSecret_（HMAC 派生金鑰）', () => {
  it('draftEncSecret 首次使用自動生成並落 ScriptProperties（與 jwtSecret 分離）', () => {
    const ctx = loadGasRpc();
    expect(ctx.store.draftEncSecret).toBe(undefined);
    const secret = ctx.gas.getDraftEncSecret_();
    expect(secret.length).toBeGreaterThanOrEqual(32);
    expect(ctx.store.draftEncSecret).toBe(secret);
    expect(secret).not.toBe(ctx.store.jwtSecret);
    // 第二次呼叫沿用同一把（不重生）
    expect(ctx.gas.getDraftEncSecret_()).toBe(secret);
  });

  it('同輸入穩定：重登永遠重算出同一把', () => {
    const ctx = loadGasRpc();
    const a = ctx.gas.deriveDraftKey_('id', REFER, '測試生甲');
    const b = ctx.gas.deriveDraftKey_('id', REFER, '測試生甲');
    expect(a).toBe(b);
    // base64url：可安全落地當 localStorage key／儲存格值
    expect(a).toMatch(/^[A-Za-z0-9_-]+$/);
  });

  it('跨 purpose / referSSID / pkey 任一不同 → 派生鍵皆不同（key separation）', () => {
    const ctx = loadGasRpc();
    const base = ctx.gas.deriveDraftKey_('id', REFER, '測試生甲');
    expect(ctx.gas.deriveDraftKey_('enc', REFER, '測試生甲')).not.toBe(base);
    expect(ctx.gas.deriveDraftKey_('log', REFER, '測試生甲')).not.toBe(base);
    expect(ctx.gas.deriveDraftKey_('id', 'OTHER_REFER', '測試生甲')).not.toBe(base);
    expect(ctx.gas.deriveDraftKey_('id', REFER, '測試生乙')).not.toBe(base);
  });
});

describe('readRecord_ 回傳 draftKeys（登入時隨 token 發兩把派生鍵）', () => {
  it('draftKeys.id / .enc 與 deriveDraftKey_ 一致且互不相同（鎖與鑰匙分離）', () => {
    const ctx = loadGasRpc();
    const result = ctx.gas.readRecord_(REFER, RECORD, [{ id: 'uid', value: '測試生甲' }]);
    expect(result).not.toBe(false);
    expect(result.token).toBeTruthy();
    expect(result.draftKeys.id).toBe(ctx.gas.deriveDraftKey_('id', REFER, '測試生甲'));
    expect(result.draftKeys.enc).toBe(ctx.gas.deriveDraftKey_('enc', REFER, '測試生甲'));
    expect(result.draftKeys.id).not.toBe(result.draftKeys.enc);
  });
});

describe('saveDraft / loadDraft（Phase 20：A 欄假名、smd1 密文直通）', () => {
  const SEALED = 'smd1:g:smv2:FAKE_CIPHERTEXT_BLOB';

  it('saveDraft：A 欄落 id 假名（非明文主鍵）、smd1 payload 原樣落地不重壓', () => {
    const ctx = loadGasRpc();
    const token = ctx.gas.issueToken_(REFER, '測試生甲', Date.now());
    const result = ctx.gas.saveDraft(REFER, token, SEALED);
    expect(result.success).toBe(true);
    // 建分頁時自帶表頭；資料列 A 欄是假名、整列找不到明文主鍵值
    expect(ctx.draftRows[0]).toEqual(ctx.gas.DRAFT_HEADER);
    const row = ctx.draftRows[1];
    expect(row[0]).toBe(ctx.gas.deriveDraftKey_('id', REFER, '測試生甲'));
    expect(row[0]).not.toContain('測試生甲');
    expect(row[2]).toBe(REFER);
    expect(row[3]).toBe(SEALED); // 不重 gzip、不動密文
  });

  it('loadDraft：以假名定位、密文原樣回傳（前端解）', () => {
    const ctx = loadGasRpc();
    const token = ctx.gas.issueToken_(REFER, '測試生甲', Date.now());
    ctx.gas.saveDraft(REFER, token, SEALED);
    const draft = ctx.gas.loadDraft(REFER, token);
    expect(draft.payload).toBe(SEALED);
    expect(typeof draft.updatedAt).toBe('number');
  });

  it('別人的 token 查不到我的草稿（假名隨 pkey 而異）', () => {
    const ctx = loadGasRpc();
    ctx.gas.saveDraft(REFER, ctx.gas.issueToken_(REFER, '測試生甲', Date.now()), SEALED);
    const other = ctx.gas.loadDraft(REFER, ctx.gas.issueToken_(REFER, '測試生乙', Date.now()));
    expect(other).toBe(null);
  });

  it('無效 token → tokenExpired（安全邊界不變）', () => {
    const ctx = loadGasRpc();
    expect(ctx.gas.saveDraft(REFER, 'bad-token', SEALED).tokenExpired).toBe(true);
    expect(ctx.gas.loadDraft(REFER, 'bad-token').tokenExpired).toBe(true);
  });
});

describe('inviteeLogin 的草稿密文（Phase 20：疊草稿移到受邀者前端）', () => {
  const SEALED = 'smd1:g:smv2:FILLER_DRAFT_BLOB';

  it('填寫者有草稿 → 回密文 blob＋後端重算的 enc key；headers 不疊草稿', () => {
    const ctx = loadGasRpc({ draftRows: [] });
    ctx.inviteRows.push(makeInviteRow(ctx.gas, {}));
    // 以真實 saveDraft 落一筆填寫者草稿（A 欄假名）
    ctx.gas.saveDraft(REFER, ctx.gas.issueToken_(REFER, '測試生甲', Date.now()), SEALED);
    const result = loginWithOtp(ctx);
    expect(result.draftBlob).toBe(SEALED);
    expect(result.draftEncKey).toBe(ctx.gas.deriveDraftKey_('enc', REFER, '測試生甲'));
    // headers 只有已送出的 savedContent，草稿密文後端解不開、不可能疊進去
    const memo = result.headers.find((column) => column.id === 'memo');
    expect(memo.savedContent).toBe('虛構測試資料');
    expect(memo.value).toBe('虛構測試資料');
    expect(memo.lastInput).toBe(undefined);
  });

  it('填寫者無草稿 → 不帶 draftBlob/draftEncKey（enc key 不無故外流）', () => {
    const ctx = loadGasRpc();
    ctx.inviteRows.push(makeInviteRow(ctx.gas, {}));
    const result = loginWithOtp(ctx);
    expect(result.draftBlob).toBe(undefined);
    expect(result.draftEncKey).toBe(undefined);
  });
});

...
| `src/utils/` | 純函數：columnRules、columnPrep、tempQueue、tempStorage、markdown、multiSelect、formatters、jwt、timeline、sheetFlow、pixelSprites——細節見 plan/struct.md |
...
- **全系統禁止 `deleteRow` / `deleteRows` / `deleteColumn`**（會位移其餘列、在 GAS 沙盒寫入不穩時容易刪錯列、且難以復原）。
  ...
  已實作為 `rebuildDraftSpreadsheet()`（Phase 18，2026-07-11）：建新暫存試算表（草稿分頁留每主鍵最新列、`_invites` 留每格
  最新快照、認不得的分頁原樣複製），舊表改名搬進備份資料夾（ScriptProperties `draftBackupFolderID`，未設不重建），翻
...
- 線上暫存已純 append 化（Phase 17，2026-07-11 實作完成）：草稿分頁為純 append 資料日誌，每次暫存
  `appendRow([主鍵, updatedAt(ms), payload...])` 一筆快照，永不 `setValues`/`clearContent`/`deleteRow`；
  「當前草稿」＝該主鍵最新一列（`latestDraftRowForKey_`，後列勝出＝舊版 superseded），無刪除/消耗概念，
  `deleteDraft` RPC 與 `draftRowIndex_` 已整支退役。第 1 列為人類可讀表頭 `DRAFT_HEADER`（凍結、對 reader
  惰性）。payload 走後端 gzip+base64 單格化（`encodeDraftPayload_`/`decodeDraftPayload_`，`gz:` 自描述前綴），
  一般草稿穩進單格；50000 字/格為 Sheets 硬限制，超長仍由 `chunkPayload_` 切塊、讀取端 C 起串接後 decode。

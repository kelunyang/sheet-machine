...
4. **暫存試算表**（`draftSheetID`，選配）：全系統共用一張，內含 `_draft`（線上暫存）、
   `_invites`（簽名邀請）、`_logins`（登入稽核）、`_file`（上傳檔案登記）、`_email`（寄信稽核）五個分頁。
   **永不對外分享**（見安全說明）。
...
| `fileLogRetentionDays` | （Phase 24）`_file` 上傳登記列的保留期（**天**，正整數），未設或非正整數＝預設 7。離線重建時清掉「問卷截止日 + 保留期」已過的登記列（守禁刪列鐵律，只在重建時清、不線上刪）；查無 referSSID 或截止日無法解析的列一律保留 |
...
- **`draftSheetID` 暫存試算表永不對外分享**：`_logins` 分頁存**明文真實登入帳號**（稽核用）、
  `_invites` 存明文主鍵、`_email` 存明文收件信箱與主鍵（寄信稽核），保護完全靠存取控制（管理者責任）。

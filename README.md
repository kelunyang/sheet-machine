...
4. **暫存試算表**（`draftSheetID`，選配）：全系統共用一張，內含 `_draft`（線上暫存）、
   `_invites`（簽名邀請）、`_logins`（登入稽核）、`_file`（上傳檔案登記）四個分頁。
   **永不對外分享**（見安全說明）。
...
| Property | 說明 |
|----------|------|
| `draftSheetID` | 暫存試算表 ID。開啟「線上暫存 + 遠端簽名邀請 + `_logins` 登入稽核 + `_file` 上傳檔案歸屬驗證」。未設＝四者停用（登入冷卻防線仍運作；檔案欄退回「相信前端傳來的 fileID」的舊行為）。**此表永不對外分享** |
